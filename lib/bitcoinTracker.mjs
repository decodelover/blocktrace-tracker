...
export async function fetchBitcoinWalletData(
  ...
  const unconfirmedSats = Array.isArray(utxos)
    ...
    : 0n
  const totalSatsFromUtxo = confirmedSats + unconfirmedSats
  const shouldUseStatsBalance =
    totalSatsFromStats >= 0n ||
    totalSatsFromUtxo === 0n ||
    totalSatsFromStats === totalSatsFromUtxo
  const liveConfirmedSats = shouldUseStatsBalance
    ? confirmedSatsFromStats
    : confirmedSats
  const liveUnconfirmedSats = shouldUseStatsBalance
    ? unconfirmedSatsFromStats
    : unconfirmedSats
  const liveTotalSats = shouldUseStatsBalance
    ? totalSatsFromStats
    : totalSatsFromUtxo
  ...
  return {
    ...
    balances: {
      confirmedSats: toSafeSatsNumber(liveConfirmedSats),
      unconfirmedSats: toSafeSatsNumber(liveUnconfirmedSats),
      totalSats: toSafeSatsNumber(liveTotalSats),
      confirmedBtc: toBtcString(liveConfirmedSats),
      unconfirmedBtc: toBtcString(liveUnconfirmedSats),
      totalBtc: toBtcString(liveTotalSats),
    },
    activity: {
